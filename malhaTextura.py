...
void main(void) 
{
    textura = texture(textureSlot,fsTextura);
}
"""
def compilaShaders():
    error = None
...
def inicializa():
    global progId, malhatexturaVAO
    GL.glEnable(GL.GL_DEPTH_TEST)
    GL.glEnable(GL.GL_MULTISAMPLE)
    progId = compilaShaders()
    malhatexturaVAO = malhatextura()
...
def desenha():
    ...
    GL.glBindVertexArray(malhatexturaVAO)
    GL.glUseProgram(progId)
...
def main():
    ...
    window = glfw.create_window(800, 600, "MalhaTextura", None, None)
    if not window:
        glfw.terminate()
        return
